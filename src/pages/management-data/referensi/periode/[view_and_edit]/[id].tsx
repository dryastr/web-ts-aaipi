...
import { update, show } from 'src/service/management-data/referensi/periode';
...
const Page: NextPage = () => {
	...
	const form: FormType[] = [
		...
		},
		{
			...
			label: 'Daerah',
			grid: [6, 6, 12],
			validationSchema: [ROLE_TYPE.SUPER_ADMIN, ROLE_TYPE.DPN].includes(user?.role_id) ? string() : string().required(`Daerah wajib diisi`),
			apiList: list,
			dataValue: 'id',
			dataLabel: 'nama',
			query: () => ({
